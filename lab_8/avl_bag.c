...
static bool a(avl_node_t* n)
{
    if (!n) return true;
    if (
        HEIGHT(n->left) > HEIGHT(n->right) + 1u || HEIGHT(n->right) > HEIGHT(n->left) + 1u
    )
        return false;
...
/*The following function inserts an element into a BST without performing any rotations*/
bool avl_insert2(avl_node_t **root, bag_elem_t elem,
                int (*cmp)(bag_elem_t, bag_elem_t))
{
    bool inserted;
...
{
    return avl_insert2(&(bag->root), elem, bag->cmp);
}


void avl_remove2(avl_node_t **root, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t))
{
    if (*root==NULL)
        return;
    
    if (cmp((*root)->elem, elem) > 0)
        avl_remove2(&((*root)->left), elem, cmp); //this call to the function is a recursive step
    else if (cmp((*root)->elem, elem) < 0)
    ...
    if (*root)
    {
        if (HEIGHT((*root)->left) > HEIGHT((*root)->right) + 1u)
            avl_rebalance_to_the_right(root);
        else if (HEIGHT((*root)->right) > HEIGHT((*root)->left) + 1u)
